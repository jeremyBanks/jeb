...
**Save after EVERY meaningful change. Intermediate states are valuable!** Even
partially-working code, failed experiments, and debug output are worth
preserving as snapshots. We can always go back to any snapshot, but we can never
...
- Labels use size-appropriate fonts (Sky < 128K, Sugimori < 512K, Mini < 1M,
  Micro < 3M)
...
- Document any non-obvious relationships between filter bytes, DEFLATE blocks,
  and ZIP structures
...
- For multi-step scripts or test files: write to a file (e.g.,
  examples/debug_foo.rs) so approval is needed once
