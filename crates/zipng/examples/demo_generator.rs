...
<body>
    ...
    <script src="app.js"></script>
        ...
        files.insert(
            b"style.css".to_vec(),
            br#"* { box-sizing: border-box; margin: 0; padding: 0; }
body { 
...
pre { background: #f5f5f5; padding: 1rem; border-radius: 6px; overflow-x: auto; }"#
        ...
        files.insert(
            b"app.js".to_vec(),
            br#"// JavaScript from inside a PNG!
document.getElementById('output').textContent = 
    `Loaded at: ${new Date().toISOString()}\n` +
    `User Agent: ${navigator.userAgent.slice(0, 50)}...`;
console.log('This JS was extracted from a PNG file!');"#
                .to_vec(),
        );
    ...
    "files": ["index.html", "style.css", "app.js"]
        ...
        println!(
            "Created webapp.png ({} bytes) - complete web application",
            polyglot.len()
        );
    }
    ...
    // Demo 2: Binary formats (simulated)
    {
        ...
        println!(
            "Created binary_mix.png ({} bytes) - various binary formats",
            polyglot.len()
        );
    }
    ...
    // Demo 3: Source code archive
    {
        ...
        files.insert(
            ...
            br#"//! A Rust program stored in a PNG
use std::env;
...
"#
        ...
        println!(
            "Created rust_project.png ({} bytes) - complete Rust project",
            polyglot.len()
        );
    }
...
"#
        ...
        println!(
            "Created polyglot_code.png ({} bytes) - multi-language hello world",
            polyglot.len()
        );
    }
...
"#
        ...
        files.insert(
            b"config.yaml".to_vec(),
            br#"# YAML config from PNG
server:
  host: localhost
  port: 8080
...
"#
        ...
        files.insert(
            b"config.toml".to_vec(),
            br#"# TOML config from PNG
[server]
host = "localhost"
port = 8080
...
"#
        ...
        files.insert(
            b".env.example".to_vec(),
            br#"# Environment variables template
...
"#
        ...
        println!(
            "Created config_bundle.png ({} bytes) - config file collection",
            polyglot.len()
        );
    }
    ...
    // Demo 6: Large mixed content (stress test)
    {
        ...
        let gradient: Vec<u8> = (0..10_000).map(|i| (i % 256) as u8).collect();
        ...
        println!(
            "Created mixed_large.png ({} bytes) - large mixed content",
            polyglot.len()
        );
    }
    ...
    // This demonstrates the removed 42KB limit
    {
        ...
        // Collect binary files from target/debug/deps (under 55KB each to fit in IDAT
        // blocks)
        let binary_sources = [
        ...
        println!(
            "Building large polyglot with {} files, {} bytes total content...",
            files.len(),
            total_size
        );
        ...
        println!(
            "Created huge.png ({} bytes / {:.1} KB) - {} files, {} KB content",
            polyglot.len(),
