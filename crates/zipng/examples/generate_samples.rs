//! Generates sample polyglot PNG+ZIP files using REAL project files and git
//! blobs.
//!
...
use {
    indexmap::IndexMap,
    std::{fs, panic::catch_unwind, process::Command},
    zipng::{
        Files, panic,
        polyglot::{
            Expectations, assert_valid_polyglot_with,
            fonts::{self, FontSelection},
        },
    },
};
...
fn main() -> Result<(), panic> {
    ...
    sample!("bitmap_fonts", vec![
        (
            "micro.png",
            include_bytes!("../src/text/micro.png").to_vec()
        ),
        (
            "micro.json",
            include_bytes!("../src/text/micro.json").to_vec()
        ),
        ("mini.png", include_bytes!("../src/text/mini.png").to_vec()),
        (
            "mini.json",
            include_bytes!("../src/text/mini.json").to_vec()
        ),
        (
            "swiss.png",
            include_bytes!("../src/text/swiss.png").to_vec()
        ),
        (
            "swiss.json",
            include_bytes!("../src/text/swiss.json").to_vec()
        ),
    ]);
    ...
    sample!("all_fonts", vec![
        ...
        (
            "sixth.png",
            include_bytes!("../src/text/sixth.png").to_vec()
        ),
        (
            "sixth.json",
            include_bytes!("../src/text/sixth.json").to_vec()
        ),
        (
            "monte.png",
            include_bytes!("../src/text/monte.png").to_vec()
        ),
        (
            "monte.json",
            include_bytes!("../src/text/monte.json").to_vec()
        ),
        ("sky.png", include_bytes!("../src/text/sky.png").to_vec()),
        ("sky.json", include_bytes!("../src/text/sky.json").to_vec()),
        (
            "sugimori.png",
            include_bytes!("../src/text/sugimori.png").to_vec()
        ),
        (
            "sugimori.json",
            include_bytes!("../src/text/sugimori.json").to_vec()
        ),
        (
            "swiss.png",
            include_bytes!("../src/text/swiss.png").to_vec()
    ...
    sample!("rust_core", vec![
        ...
        (
            "checksums.rs",
            include_bytes!("../src/checksums.rs").to_vec()
        ),
    ...
    sample!("png_module", vec![
        ...
        (
            "png/to_png.rs",
            include_bytes!("../src/png/to_png.rs").to_vec()
        ),
        (
            "png/write_png.rs",
            include_bytes!("../src/png/write_png.rs").to_vec()
        ),
        (
            "png/sizes.rs",
            include_bytes!("../src/png/sizes.rs").to_vec()
        ),
    ]);
    ...
    sample!("zip_module", vec![
        ...
        (
            "zip/to_zip.rs",
            include_bytes!("../src/zip/to_zip.rs").to_vec()
        ),
        (
            "zip/write_zip.rs",
            include_bytes!("../src/zip/write_zip.rs").to_vec()
        ),
        (
            "zip/configuration.rs",
            include_bytes!("../src/zip/configuration.rs").to_vec()
        ),
    ]);
    ...
    sample!("polyglot_support", vec![
        (
            "polyglot/fonts.rs",
            include_bytes!("../src/polyglot/fonts.rs").to_vec()
        ),
        (
            "polyglot/validate.rs",
            include_bytes!("../src/polyglot/validate.rs").to_vec()
        ),
    ]);
    ...
    sample!("palettes", vec![
        (
            "palettes/mod.rs",
            include_bytes!("../src/png/palettes.rs").to_vec()
        ),
        (
            "palettes/viridis.rs",
            include_bytes!("../src/png/palettes/viridis.rs").to_vec()
        ),
        (
            "palettes/singles.rs",
            include_bytes!("../src/png/palettes/singles.rs").to_vec()
        ),
        (
            "palettes/diagnostic.rs",
            include_bytes!("../src/png/palettes/diagnostic.rs").to_vec()
        ),
        (
            "palettes/mappings.rs",
            include_bytes!("../src/png/palettes/mappings.rs").to_vec()
        ),
    ]);
    ...
    sample!("documentation", vec![
        (
            "docs/polyglot-architecture.md",
            include_bytes!("../docs/polyglot-architecture.md").to_vec()
        ),
        (
            "docs/polyglot-constraints.md",
            include_bytes!("../docs/polyglot-constraints.md").to_vec()
        ),
    ]);
    ...
    sample!("test_data", vec![
        (
            "test_data/zip.zip",
            include_bytes!("../test_data/zip.zip").to_vec()
        ),
        (
            "test_data/zip.htm",
            include_bytes!("../test_data/zip.htm").to_vec()
        ),
        (
            "test_data/poc.png.zip",
            include_bytes!("../test_data/poc.png.zip").to_vec()
        ),
    ]);
    ...
    sample!("examples", vec![
        ...
        (
            "examples/indexed8bit.rs",
            include_bytes!("indexed8bit.rs").to_vec()
        ),
        (
            "examples/greyscale8bit.rs",
            include_bytes!("greyscale8bit.rs").to_vec()
        ),
    ]);
    ...
    sample!("fic_source", vec![
        (
            "src/lib.rs",
            git_blob("4891687bac3382f7eb2c34fe6412d8697a618739")
        ),
        (
            "src/backend.rs",
            git_blob("6a6b07165b1cd913d2e788861bc9686c68221863")
        ),
        (
            "src/engine.rs",
            git_blob("65ba75b3b14035ba93d624642a6003c4ed240e82")
        ),
        (
            "src/query.rs",
            git_blob("8cf2bf7dbf5088a7e4a639d0cd98db0406d9b95f")
        ),
    ]);
    ...
    sample!("mixed_project", vec![
        (
            "README.md",
            git_blob("4dc6ed1606381e6e6555b3cfbd01ae2d1aa869ed")
        ),
        ("Cargo.toml", include_bytes!("../../../Cargo.toml").to_vec()),
        ("CLAUDE.md", include_bytes!("../../../CLAUDE.md").to_vec()),
    ]);
    ...
    sample!("vscode_configs", vec![
        (
            ...
            include_bytes!("../.vscode/settings.json").to_vec()
        ),
        (
            ".vscode/launch.json",
            include_bytes!("../.vscode/launch.json").to_vec()
        ),
    ]);
    ...
    sample!("scripts", vec![
        (
            "scripts/debug_polyglot.rs",
            include_bytes!("../scripts/debug_polyglot.rs").to_vec()
        ),
        (
            "scripts/check_font.rs",
            include_bytes!("../scripts/check_font.rs").to_vec()
        ),
        (
            "scripts/test_kerning.rs",
            include_bytes!("../scripts/test_kerning.rs").to_vec()
        ),
    ]);
    ...
    sample!("io_module", vec![
        ...
        (
            "io/alignment.rs",
            include_bytes!("../src/io/alignment.rs").to_vec()
        ),
    ]);
    ...
    sample!("exploration_docs", vec![
        (
            "idat-boundary-analysis.md",
            include_bytes!("../docs/exploration/idat-boundary-analysis.md").to_vec()
        ),
        (
            "variable-width-math.md",
            include_bytes!("../docs/exploration/variable-width-math.md").to_vec()
        ),
        (
            "variable-width-plan.md",
            include_bytes!("../docs/exploration/variable-width-plan.md").to_vec()
        ),
        (
            "wider-images-analysis.md",
            include_bytes!("../docs/exploration/wider-images-analysis.md").to_vec()
        ),
    ]);
    ...
    // NEW: Binary mix — woff2 fonts + favicon + cover + zip files (all from git
    // blobs)
    sample!("binary_mix", vec![
        (
            "sans701.woff2",
            git_blob("43f253e52c22ddc961267a7422f4f0a3fbdad3ff")
        ),
        (
            "sans401.woff2",
            git_blob("d35d3a78dc5df918f102b566115f22845bfcce31")
        ),
        (
            "sans700.woff2",
            git_blob("19a58eace2f37f645b2e724046288a979efafd84")
        ),
        (
            "sans400.woff2",
            git_blob("99b3c6f5e440bc94bb70b1727928066cadfd1bbf")
        ),
        (
            "favicon.ico",
            git_blob("b99cca8acdeaab2246318b3489fdda5c9544f425")
        ),
        (
            "cover.png",
            git_blob("b3fc3f510f4f6c0b832bdc04c31daeab28602412")
        ),
        (
            "zip.zip",
            git_blob("00f315606dd99dd49565bdd92ccbd72f32ddca9f")
        ),
        (
            "default.zip",
            git_blob("434715bf08cdbbc21eda2a3c0bad1f8d1277344f")
        ),
    ]);
    ...
    sample!("images", vec![
        (
            "hybrid.png",
            git_blob("4f77dc8bc5201e9b4ae092b4c569b1cbac0aedb8")
        ),
        (
            "revolution.png",
            git_blob("cb4d4bb40c47238570b7fe0ce8ae5980ef7b7c6b")
        ),
        (
            "icon.png",
            git_blob("518ab5baa722247ee4b9311c43f251abdf7f908e")
        ),
        (
            "cover.png",
            git_blob("b3fc3f510f4f6c0b832bdc04c31daeab28602412")
        ...
        (
            "poc-gray.png",
            git_blob("ef554fe90423bdd8212bf1c1b7fb31470c612ba9")
        ),
        (
            "poc-turbo.png",
            git_blob("7d17ae102570957cfd7a9acfdee22088efa50219")
        ),
        (
            "sizes-2048.png",
            git_blob("cda9599db284f7bc413a8e0990f6e896e83ef829")
        ),
        (
            "fantasy14pt.png",
            git_blob("af47e0675796fd5e5e48f74230c83c6c91c1d316")
        ),
        (
            "sans7pt.png",
            git_blob("a961936492531d211497582601589f13704dd3f1")
        ),
        (
            "sizes-512.png",
            git_blob("36c049e6e23520140698f5e79888053473a179dc")
        ),
        (
            "sizes-32.png",
            git_blob("bf483fdaa9ad9febe5e27023260e9e8370b48779")
        ),
        (
            "default.png",
            git_blob("d076cc17619ce89823a22bf8ac2186211a89e39d")
        ),
    ]);
    ...
    sample!("web_app", vec![
        (
            "ChapterPlayer.tsx",
            git_blob("6775460dc8205391fcfe8e2fcba1376fb4585d71")
        ),
        (
            "rss.ts",
            git_blob("b4bc50599277271eb20ab3ebaba00b5580078b7d")
        ),
        (
            "oklab.ts",
            git_blob("edc33782a6fcb821b3146fb8cad50fb72282b0c6")
        ),
        (
            "tts.rs",
            git_blob("417a417289e71d71159c54cd8a1bf3f761b2e8ff")
        ),
    ]);
...

    {
        let files: Vec<(&str, Vec<u8>)> = vec![
            ...
            (
                "history/RYL0051925.json",
                git_blob("6429e34ed36a6cc563167626d89973d64ab538d9"),
            ),
            (
                "history/RYL0021220.json",
                git_blob("8568558f3115ccd2859741b0b7ea1822beb169b5"),
            ),
            (
                "history/poc.htm",
                git_blob("5e2c1c3f950b74db76c34df838363d6a6be2c55a"),
            ),
            (
                "history/zip.xml",
                git_blob("9be3db2625f4ebc85ed17078eb22ecacf687071f"),
            ),
            // Historic polyglot/mod.rs versions (large, under 60KB)
            (
                "history/polyglot_v1.rs",
                git_blob("105696cda2fa45fa53c98b403dc681f9d3ffd2d9"),
            ),
            (
                "history/polyglot_v2.rs",
                git_blob("77e6918a8f3b0128ea404045c323398f464914d5"),
            ),
            (
                "history/polyglot_v3.rs",
                git_blob("da359b39faf049612742a212af282893e096bfe9"),
            ),
            // Historic source versions
            (
                "history/write_zipng_v1.rs",
                git_blob("580135c539bc0138087100c1dabcb0f7b7577592"),
            ),
            (
                "history/write_zipng_v2.rs",
                git_blob("fc2ebd0b541bf25d0459c57dfc9392e26442418b"),
            ),
            (
                "history/png_v1.rs",
                git_blob("01e49c4108c13a34a3fed61037916ac4dd98b401"),
            ),
            (
                "history/png_v2.rs",
                git_blob("ec87a5b3261fd6faf07c94929ee1f8cad5d386b5"),
            ),
            (
                "history/zip_v1.rs",
                git_blob("649a65c68e685b194c03bbd97d942bae2914bd6f"),
            ),
            // Binary blobs
            (
                "assets/hybrid.png",
            ...
            (
                "assets/sans701.woff2",
                git_blob("43f253e52c22ddc961267a7422f4f0a3fbdad3ff"),
            ),
            (
                "assets/sans401.woff2",
                git_blob("d35d3a78dc5df918f102b566115f22845bfcce31"),
            ),
            (
                "assets/sans700.woff2",
                git_blob("19a58eace2f37f645b2e724046288a979efafd84"),
            ),
            (
                "assets/sans400.woff2",
                git_blob("99b3c6f5e440bc94bb70b1727928066cadfd1bbf"),
            ),
            (
                "assets/favicon.ico",
            ...
            (
                "current/checksums.rs",
                include_bytes!("../src/checksums.rs").to_vec(),
            ),
            (
                "current/deflate.rs",
                include_bytes!("../src/deflate.rs").to_vec(),
            ),
            ("current/zlib.rs", include_bytes!("../src/zlib.rs").to_vec()),
            (
                "current/png/mod.rs",
                include_bytes!("../src/png.rs").to_vec(),
            ),
            (
                "current/png/data.rs",
                include_bytes!("../src/png/data.rs").to_vec(),
            ),
            (
                "current/png/to_png.rs",
                include_bytes!("../src/png/to_png.rs").to_vec(),
            ),
            (
                "current/png/write_png.rs",
                include_bytes!("../src/png/write_png.rs").to_vec(),
            ),
            (
                "current/png/sizes.rs",
                include_bytes!("../src/png/sizes.rs").to_vec(),
            ),
            (
                "current/zip/mod.rs",
                include_bytes!("../src/zip.rs").to_vec(),
            ),
            (
                "current/zip/data.rs",
                include_bytes!("../src/zip/data.rs").to_vec(),
            ),
            (
                "current/zip/to_zip.rs",
                include_bytes!("../src/zip/to_zip.rs").to_vec(),
            ),
            (
                "current/zip/write_zip.rs",
                include_bytes!("../src/zip/write_zip.rs").to_vec(),
            ),
            (
                "current/zip/configuration.rs",
                include_bytes!("../src/zip/configuration.rs").to_vec(),
            ),
            (
                "current/polyglot/fonts.rs",
                include_bytes!("../src/polyglot/fonts.rs").to_vec(),
            ),
            (
                "current/polyglot/validate.rs",
                include_bytes!("../src/polyglot/validate.rs").to_vec(),
            ),
            ("current/io/mod.rs", include_bytes!("../src/io.rs").to_vec()),
            (
                "current/io/alignment.rs",
                include_bytes!("../src/io/alignment.rs").to_vec(),
            ),
            (
                "current/palettes/mod.rs",
                include_bytes!("../src/png/palettes.rs").to_vec(),
            ),
            (
                "current/palettes/viridis.rs",
                include_bytes!("../src/png/palettes/viridis.rs").to_vec(),
            ),
            (
                "current/palettes/singles.rs",
                include_bytes!("../src/png/palettes/singles.rs").to_vec(),
            ),
            (
                "current/palettes/diagnostic.rs",
                include_bytes!("../src/png/palettes/diagnostic.rs").to_vec(),
            ),
            (
                "current/palettes/mappings.rs",
                include_bytes!("../src/png/palettes/mappings.rs").to_vec(),
            ),
            // Config
            ("Cargo.toml", include_bytes!("../Cargo.toml").to_vec()),
            ...
            (
                "docs/idat-boundary-analysis.md",
                include_bytes!("../docs/exploration/idat-boundary-analysis.md").to_vec(),
            ),
            (
                "docs/variable-width-math.md",
                include_bytes!("../docs/exploration/variable-width-math.md").to_vec(),
            ),
            (
                "docs/variable-width-plan.md",
                include_bytes!("../docs/exploration/variable-width-plan.md").to_vec(),
            ),
            (
                "docs/wider-images-analysis.md",
                include_bytes!("../docs/exploration/wider-images-analysis.md").to_vec(),
            ),
        ];
...

    {
        let files: Vec<(&str, Vec<u8>)> = vec![
            ...
            (
                "assets/poc-turbo.png",
                git_blob("7d17ae102570957cfd7a9acfdee22088efa50219"),
            ),
            (
                "assets/sizes-2048.png",
                git_blob("cda9599db284f7bc413a8e0990f6e896e83ef829"),
            ),
            (
                "assets/sans701.woff2",
            ...
            (
                "assets/poc.png.zip",
                git_blob("7d5e5c629b918f8dcbec25164b12484e949253dd"),
            ),
            // Snapshot A — current zipng source
            (
                "snapshot-a/lib.rs",
                include_bytes!("../src/lib.rs").to_vec(),
            ),
            (
                "snapshot-a/checksums.rs",
                include_bytes!("../src/checksums.rs").to_vec(),
            ),
            (
                "snapshot-a/deflate.rs",
                include_bytes!("../src/deflate.rs").to_vec(),
            ),
            (
                "snapshot-a/zlib.rs",
                include_bytes!("../src/zlib.rs").to_vec(),
            ),
            (
                "snapshot-a/png/mod.rs",
                include_bytes!("../src/png.rs").to_vec(),
            ),
            (
                "snapshot-a/png/data.rs",
                include_bytes!("../src/png/data.rs").to_vec(),
            ),
            (
                "snapshot-a/png/to_png.rs",
                include_bytes!("../src/png/to_png.rs").to_vec(),
            ),
            (
                "snapshot-a/png/write_png.rs",
                include_bytes!("../src/png/write_png.rs").to_vec(),
            ),
            (
                "snapshot-a/png/sizes.rs",
                include_bytes!("../src/png/sizes.rs").to_vec(),
            ),
            (
                "snapshot-a/zip/mod.rs",
                include_bytes!("../src/zip.rs").to_vec(),
            ),
            (
                "snapshot-a/zip/data.rs",
                include_bytes!("../src/zip/data.rs").to_vec(),
            ),
            (
                "snapshot-a/zip/to_zip.rs",
                include_bytes!("../src/zip/to_zip.rs").to_vec(),
            ),
            (
                "snapshot-a/zip/write_zip.rs",
                include_bytes!("../src/zip/write_zip.rs").to_vec(),
            ),
            (
                "snapshot-a/zip/configuration.rs",
                include_bytes!("../src/zip/configuration.rs").to_vec(),
            ),
            (
                "snapshot-a/polyglot/fonts.rs",
                include_bytes!("../src/polyglot/fonts.rs").to_vec(),
            ),
            (
                "snapshot-a/polyglot/validate.rs",
                include_bytes!("../src/polyglot/validate.rs").to_vec(),
            ),
            (
                "snapshot-a/io/mod.rs",
                include_bytes!("../src/io.rs").to_vec(),
            ),
            (
                "snapshot-a/io/alignment.rs",
                include_bytes!("../src/io/alignment.rs").to_vec(),
            ),
            (
                "snapshot-a/palettes/mod.rs",
                include_bytes!("../src/png/palettes.rs").to_vec(),
            ),
            (
                "snapshot-a/palettes/viridis.rs",
                include_bytes!("../src/png/palettes/viridis.rs").to_vec(),
            ),
            (
                "snapshot-a/palettes/singles.rs",
                include_bytes!("../src/png/palettes/singles.rs").to_vec(),
            ),
            (
                "snapshot-a/palettes/diagnostic.rs",
                include_bytes!("../src/png/palettes/diagnostic.rs").to_vec(),
            ),
            (
                "snapshot-a/palettes/mappings.rs",
                include_bytes!("../src/png/palettes/mappings.rs").to_vec(),
            ),
            ...
            (
                "snapshot-a2/checksums.rs",
                include_bytes!("../src/checksums.rs").to_vec(),
            ),
            (
                "snapshot-a2/deflate.rs",
                include_bytes!("../src/deflate.rs").to_vec(),
            ),
            (
                "snapshot-a2/zlib.rs",
            ...
            (
                "snapshot-a2/png/mod.rs",
                include_bytes!("../src/png.rs").to_vec(),
            ),
            (
                "snapshot-a2/png/data.rs",
                include_bytes!("../src/png/data.rs").to_vec(),
            ),
            (
                "snapshot-a2/png/to_png.rs",
                include_bytes!("../src/png/to_png.rs").to_vec(),
            ),
            (
                "snapshot-a2/png/write_png.rs",
                include_bytes!("../src/png/write_png.rs").to_vec(),
            ),
            (
                "snapshot-a2/png/sizes.rs",
                include_bytes!("../src/png/sizes.rs").to_vec(),
            ),
            (
                "snapshot-a2/zip/mod.rs",
                include_bytes!("../src/zip.rs").to_vec(),
            ),
            (
                "snapshot-a2/zip/data.rs",
                include_bytes!("../src/zip/data.rs").to_vec(),
            ),
            (
                "snapshot-a2/zip/to_zip.rs",
                include_bytes!("../src/zip/to_zip.rs").to_vec(),
            ),
            (
                "snapshot-a2/zip/write_zip.rs",
                include_bytes!("../src/zip/write_zip.rs").to_vec(),
            ),
            (
                "snapshot-a2/zip/configuration.rs",
                include_bytes!("../src/zip/configuration.rs").to_vec(),
            ),
            (
                "snapshot-a2/polyglot/fonts.rs",
                include_bytes!("../src/polyglot/fonts.rs").to_vec(),
            ),
            (
                "snapshot-a2/polyglot/validate.rs",
                include_bytes!("../src/polyglot/validate.rs").to_vec(),
            ),
            (
                "snapshot-a2/io/mod.rs",
                include_bytes!("../src/io.rs").to_vec(),
            ),
            (
                "snapshot-a2/io/alignment.rs",
                include_bytes!("../src/io/alignment.rs").to_vec(),
            ),
            (
                "snapshot-a2/palettes/mod.rs",
                include_bytes!("../src/png/palettes.rs").to_vec(),
            ),
            (
                "snapshot-a2/palettes/viridis.rs",
                include_bytes!("../src/png/palettes/viridis.rs").to_vec(),
            ),
            (
                "snapshot-a2/palettes/singles.rs",
                include_bytes!("../src/png/palettes/singles.rs").to_vec(),
            ),
            (
                "snapshot-a2/palettes/diagnostic.rs",
                include_bytes!("../src/png/palettes/diagnostic.rs").to_vec(),
            ),
            (
                "snapshot-a2/palettes/mappings.rs",
                include_bytes!("../src/png/palettes/mappings.rs").to_vec(),
            ),
            // Snapshot B — parent project + config + docs + examples
            (
                "snapshot-b/fic/lib.rs",
                git_blob("4891687bac3382f7eb2c34fe6412d8697a618739"),
            ),
            (
                "snapshot-b/fic/backend.rs",
                git_blob("6a6b07165b1cd913d2e788861bc9686c68221863"),
            ),
            (
                "snapshot-b/fic/engine.rs",
                git_blob("65ba75b3b14035ba93d624642a6003c4ed240e82"),
            ),
            (
                "snapshot-b/fic/query.rs",
                git_blob("8cf2bf7dbf5088a7e4a639d0cd98db0406d9b95f"),
            ),
            (
                "snapshot-b/Cargo.toml",
                include_bytes!("../Cargo.toml").to_vec(),
            ),
            (
                "snapshot-b/Cargo.lock",
                include_bytes!("../Cargo.lock").to_vec(),
            ),
            (
                "snapshot-b/README.md",
                include_bytes!("../README.md").to_vec(),
            ),
            (
                "snapshot-b/CLAUDE.md",
                include_bytes!("../CLAUDE.md").to_vec(),
            ),
            (
                "snapshot-b/examples/zipng.rs",
                include_bytes!("zipng.rs").to_vec(),
            ),
            (
                "snapshot-b/examples/rgb.rs",
                include_bytes!("rgb.rs").to_vec(),
            ),
            (
                "snapshot-b/examples/indexed8bit.rs",
                include_bytes!("indexed8bit.rs").to_vec(),
            ),
            (
                "snapshot-b/examples/greyscale8bit.rs",
                include_bytes!("greyscale8bit.rs").to_vec(),
            ),
            (
                "snapshot-b/scripts/debug_polyglot.rs",
                include_bytes!("../scripts/debug_polyglot.rs").to_vec(),
            ),
            (
                "snapshot-b/scripts/check_font.rs",
                include_bytes!("../scripts/check_font.rs").to_vec(),
            ),
            (
                "snapshot-b/scripts/test_kerning.rs",
                include_bytes!("../scripts/test_kerning.rs").to_vec(),
            ),
            // Snapshot C — docs
            (
                "snapshot-c/polyglot-architecture.md",
                include_bytes!("../docs/polyglot-architecture.md").to_vec(),
            ),
            (
                "snapshot-c/polyglot-constraints.md",
                include_bytes!("../docs/polyglot-constraints.md").to_vec(),
            ),
            (
                "snapshot-c/idat-boundary-analysis.md",
                include_bytes!("../docs/exploration/idat-boundary-analysis.md").to_vec(),
            ),
            (
                "snapshot-c/variable-width-math.md",
                include_bytes!("../docs/exploration/variable-width-math.md").to_vec(),
            ),
            (
                "snapshot-c/variable-width-plan.md",
                include_bytes!("../docs/exploration/variable-width-plan.md").to_vec(),
            ),
            (
                "snapshot-c/wider-images-analysis.md",
                include_bytes!("../docs/exploration/wider-images-analysis.md").to_vec(),
            ),
            ...
            (
                "archive/RYL0051925.json",
                git_blob("6429e34ed36a6cc563167626d89973d64ab538d9"),
            ),
            (
                "archive/RYL0021220.json",
                git_blob("8568558f3115ccd2859741b0b7ea1822beb169b5"),
            ),
            (
                "archive/polyglot_v2.rs",
                git_blob("77e6918a8f3b0128ea404045c323398f464914d5"),
            ),
            (
                "archive/polyglot_v3.rs",
                git_blob("da359b39faf049612742a212af282893e096bfe9"),
            ),
        ];
    ...
    // =========================================================================
    // RGBA with labels (2-3M content: triggers RGBA mode but stays under label
    // threshold)
    // =========================================================================
        ...
        if content_so_far < target {
            ...
            for i in 0..chunks {
                let size = if i < chunks - 1 {
                    ...
                    remaining - i * chunk_size
                };
                let name: &'static str =
                    Box::leak(format!("padding/pad_{i:03}.bin").into_boxed_str());
...

    {
        ...
        let source_snapshot: Vec<(&str, &[u8])> = vec![
            ...
            (
                "png/write_png.rs",
                include_bytes!("../src/png/write_png.rs"),
            ),
            ...
            (
                "zip/write_zip.rs",
                include_bytes!("../src/zip/write_zip.rs"),
            ),
            (
                "zip/configuration.rs",
                include_bytes!("../src/zip/configuration.rs"),
            ),
            (
                "polyglot/fonts.rs",
                include_bytes!("../src/polyglot/fonts.rs"),
            ),
            (
                "polyglot/validate.rs",
                include_bytes!("../src/polyglot/validate.rs"),
            ),
            ...
            (
                "palettes/viridis.rs",
                include_bytes!("../src/png/palettes/viridis.rs"),
            ),
            (
                "palettes/singles.rs",
                include_bytes!("../src/png/palettes/singles.rs"),
            ),
            (
                "palettes/diagnostic.rs",
                include_bytes!("../src/png/palettes/diagnostic.rs"),
            ),
            (
                "palettes/mappings.rs",
                include_bytes!("../src/png/palettes/mappings.rs"),
            ),
        ];
        ...
        let history_blobs: Vec<(&str, &str)> = vec![
            ...
            (
                "ChapterPlayer.tsx",
                "6775460dc8205391fcfe8e2fcba1376fb4585d71",
            ),
            ("rss.ts", "b4bc50599277271eb20ab3ebaba00b5580078b7d"),
            ("oklab.ts", "edc33782a6fcb821b3146fb8cad50fb72282b0c6"),
            (
                "RYL0051925.json",
                "6429e34ed36a6cc563167626d89973d64ab538d9",
            ),
            (
                "RYL0021220.json",
                "8568558f3115ccd2859741b0b7ea1822beb169b5",
            ),
        ...
        for (name, hash) in [
            (
                "assets/hybrid.png",
                "4f77dc8bc5201e9b4ae092b4c569b1cbac0aedb8",
            ),
            (
                "assets/revolution.png",
                "cb4d4bb40c47238570b7fe0ce8ae5980ef7b7c6b",
            ),
            (
                "assets/icon.png",
                "518ab5baa722247ee4b9311c43f251abdf7f908e",
            ),
            (
                "assets/cover.png",
                "b3fc3f510f4f6c0b832bdc04c31daeab28602412",
            ),
            (
                "assets/poc-gray.png",
                "ef554fe90423bdd8212bf1c1b7fb31470c612ba9",
            ),
            (
                "assets/poc-turbo.png",
                "7d17ae102570957cfd7a9acfdee22088efa50219",
            ),
            (
                "assets/sizes-2048.png",
                "cda9599db284f7bc413a8e0990f6e896e83ef829",
            ),
            (
                "assets/fantasy14pt.png",
                "af47e0675796fd5e5e48f74230c83c6c91c1d316",
            ),
            (
                "assets/sans7pt.png",
                "a961936492531d211497582601589f13704dd3f1",
            ),
            (
                "assets/sans701.woff2",
                "43f253e52c22ddc961267a7422f4f0a3fbdad3ff",
            ),
            (
                "assets/sans401.woff2",
                "d35d3a78dc5df918f102b566115f22845bfcce31",
            ),
            (
                "assets/sans700.woff2",
                "19a58eace2f37f645b2e724046288a979efafd84",
            ),
            (
                "assets/sans400.woff2",
                "99b3c6f5e440bc94bb70b1727928066cadfd1bbf",
            ),
            (
                "assets/favicon.ico",
                "b99cca8acdeaab2246318b3489fdda5c9544f425",
            ),
        ] {
        ...
        for (name, data) in [
            ...
            (
                "config/CLAUDE.md",
                include_bytes!("../CLAUDE.md").as_slice(),
            ),
            (
                "config/rustfmt.toml",
                include_bytes!("../rustfmt.toml").as_slice(),
            ),
            (
                "docs/polyglot-architecture.md",
                include_bytes!("../docs/polyglot-architecture.md").as_slice(),
            ),
            (
                "docs/polyglot-constraints.md",
                include_bytes!("../docs/polyglot-constraints.md").as_slice(),
            ),
            (
                "docs/idat-boundary-analysis.md",
                include_bytes!("../docs/exploration/idat-boundary-analysis.md").as_slice(),
            ),
            (
                "docs/variable-width-math.md",
                include_bytes!("../docs/exploration/variable-width-math.md").as_slice(),
            ),
            (
                "docs/variable-width-plan.md",
                include_bytes!("../docs/exploration/variable-width-plan.md").as_slice(),
            ),
            (
                "docs/wider-images-analysis.md",
                include_bytes!("../docs/exploration/wider-images-analysis.md").as_slice(),
            ),
            ("examples/zipng.rs", include_bytes!("zipng.rs").as_slice()),
            ("examples/rgb.rs", include_bytes!("rgb.rs").as_slice()),
            (
                "examples/indexed8bit.rs",
                include_bytes!("indexed8bit.rs").as_slice(),
            ),
            (
                "examples/greyscale8bit.rs",
                include_bytes!("greyscale8bit.rs").as_slice(),
            ),
            (
                "scripts/debug_polyglot.rs",
                include_bytes!("../scripts/debug_polyglot.rs").as_slice(),
            ),
            (
                "scripts/check_font.rs",
                include_bytes!("../scripts/check_font.rs").as_slice(),
            ),
            (
                "scripts/test_kerning.rs",
                include_bytes!("../scripts/test_kerning.rs").as_slice(),
            ),
        ] {
        ...
        for (name, hash) in [
            ...
            (
                "parent/backend.rs",
                "6a6b07165b1cd913d2e788861bc9686c68221863",
            ),
            (
                "parent/engine.rs",
                "65ba75b3b14035ba93d624642a6003c4ed240e82",
            ),
            (
                "parent/query.rs",
                "8cf2bf7dbf5088a7e4a639d0cd98db0406d9b95f",
            ),
        ] {
            files.push((name.to_string(), git_blob(hash)));
        }
    ...
    // 1. All files just over half bucket size — every file gets its own bucket
    {
        let files: Vec<(&str, Vec<u8>)> = (0..8)
            .map(|i| {
                ...
                let content: Vec<u8> = (0..33_000u32)
    ...
    // 2. Half-plus files mixed with small files that fill the gaps
    {
        ...
        for i in 0..6 {
            ...
            let content: Vec<u8> = (0..34_000u32)
                .map(|j| ((i * 41 + j * 11) % 256) as u8)
                .collect();
    ...
    // 3. Files at exactly varying fractions of bucket size (1/3, 1/2+, 2/3) to
    //    create irregular gap patterns
    {
        let files: Vec<(&str, Vec<u8>)> = vec![
    ...
    // 4. Near-max files (~58KB) that barely fit one per bucket, with small files
    //    that can only squeeze into leftover scraps
    {
        let mut files: Vec<(&str, Vec<u8>)> = Vec::new();
        for i in 0..4 {
            let name: &'static str = Box::leak(format!("huge_{i}.bin").into_boxed_str());
            let content: Vec<u8> = (0..58_000u32)
                .map(|j| ((i * 53 + j * 3) % 256) as u8)
                .collect();
            files.push((name, content));
        }
        for i in 0..20 {
    ...
    // 5. Many half-plus files with random sub-half files — larger scale stress
    {
        ...
        for i in 0..10 {
            ...
            let content: Vec<u8> = (0..size as u32)
                .map(|j| ((i as u32 * 43 + j * 17) % 256) as u8)
                .collect();
            files.push((name, content));
        }
        for i in 0..15 {
            ...
            let content: Vec<u8> = (0..size as u32)
                .map(|j| ((i as u32 * 31 + j * 9) % 256) as u8)
                .collect();
            files.push((name, content));
        }
...

    {
        ...
        let sorted_files: Vec<(&[u8], &[u8])> = {
            let mut v: Vec<(&[u8], &[u8])> = png_module_files
                .iter()
        ...
        for (palette_name, palette_data) in palette_variants {
            ...
            fs::write(&path, &polyglot)?;
            println!(
                "  variant: color_{palette_name}.png ({} bytes)",
                polyglot.len()
            );
        }
        ...
        println!(
            "Generated {} font + {} color variants in target/samples/png_module/",
            font_variants.len(),
            palette_variants.len() + 1
        );
    }
