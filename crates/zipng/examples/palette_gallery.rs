...
use {
    rgb::RGB8,
    std::{fs, path::Path},
    zipng::{palettes, palettes::perceptual},
};
...
fn main() {
    ...
    let perceptual_sets: Vec<(&str, Vec<RGB8>)> = vec![
        ...
        // ── 3 colors ────────────────────────────────────────────────
        ("03-white-blue-black", vec![
            c(0xFF, 0xFF, 0xFF),
            c(0x3B, 0x82, 0xF6),
            c(0x00, 0x00, 0x00),
        ]),
        ("03-black-red-white", vec![
            c(0x00, 0x00, 0x00),
            c(0xFF, 0x00, 0x00),
            c(0xFF, 0xFF, 0xFF),
        ]),
    ...
    println!(
        "\nGenerated {total} palette images in {}",
        out_dir.display()
    );
}
