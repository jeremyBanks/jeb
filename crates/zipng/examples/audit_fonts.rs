...
fn main() {
    let fonts: Vec<(&str, &[u8], &[u8])> = vec![
        (
            "sixth",
            include_bytes!("../src/text/sixth.json"),
            include_bytes!("../src/text/sixth.png"),
        ),
        (
            "swiss",
            include_bytes!("../src/text/swiss.json"),
            include_bytes!("../src/text/swiss.png"),
        ),
        (
            "sky",
            include_bytes!("../src/text/sky.json"),
            include_bytes!("../src/text/sky.png"),
        ),
        (
            "monte",
            include_bytes!("../src/text/monte.json"),
            include_bytes!("../src/text/monte.png"),
        ),
        (
            "sugimori",
            include_bytes!("../src/text/sugimori.json"),
            include_bytes!("../src/text/sugimori.png"),
        ),
        (
            "mini",
            include_bytes!("../src/text/mini.json"),
            include_bytes!("../src/text/mini.png"),
        ),
        (
            "micro",
            include_bytes!("../src/text/micro.json"),
            include_bytes!("../src/text/micro.png"),
        ),
    ];
...
fn audit_font(file_name: &str, json_data: &[u8], png_data: &[u8]) {
    ...
    if !dark_outside.is_empty() {
        ...
        for &(x, y) in dark_outside.iter().take(50) {
            ...
            println!(
                "    pixel ({},{}) outside cell, nearest glyph '{}' at ({},{}) cell=[{},{}..{},{}]",
                x,
                y,
                ch,
                gx,
    ...
    if !empty_glyphs.is_empty() {
        println!(
            "  *** {} non-space glyphs with NO dark pixels ***",
            empty_glyphs.len()
        );
