...
fn main() {
    ...
    // `::encode` and `.encode` take a very generic input type of basically anything
    // that can be iterated over as pairs of values that can each be borrowed as
    // `&[u8]`, which may be different types.
    ...
    // zipng::v2::decode(&encoded_with_defaults); let decoded_with_config =
    // zipng::v2::Decoder::new()     .with_source(zipng::v2::Source::Zip)
