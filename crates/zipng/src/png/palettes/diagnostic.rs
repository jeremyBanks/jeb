//! Custom diagnostic palettes for visualizing binary data.
//!
//! These palettes are designed to make byte values visually distinguishable
//! when viewing ZIP or other binary data as PNG images.

/// A 256-color palette designed for visualizing binary data.
///
/// Color scheme:
/// - `0x00` (null): Black
/// - `0x01`-`0x08`, `0x0B`-`0x0C`, `0x0E`-`0x1F`: Dark teal (`0xXX, 0x80, 0x20`)
/// - `0x09`-`0x0A`, `0x0D` (whitespace): Dark gray
/// - `0x20` (space): Gray
/// - `0x21`-`0x7E` (printable ASCII): Bright magenta-ish (`0xXX, 0xFF, 0x80`)
/// - `0x7F`: Teal (`0x7F, 0x80, 0x20`)
/// - `0x80`-`0xFF` (high bytes): Purple-ish (`0xXX, 0x40, 0x80`)
#[rustfmt::skip]
pub const BYTE_VALUE: &[u8; 256 * 3] = &[
...
/// A lookup table mapping byte values to indices sorted by bit density (popcount).
///
/// This can be used to reorder bytes for visualization where bytes with similar
/// bit patterns appear near each other, creating smoother gradients in binary data.
#[rustfmt::skip]
...
/// Alias for backwards compatibility with original name.
pub const PALLETTE_8_BIT_DATA: &[u8; 256 * 3] = BYTE_VALUE;
