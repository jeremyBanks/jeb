//! Test one complete division iteration

use zerodmg_codes::instruction::prelude::*;
use zerodmg_codes::instruction::FlagCondition;

fn main() {
    let rom = build_rom();
    std::fs::write("test-one-iter.gb", &rom).expect("Failed to write ROM");
    println!("Generated test-one-iter.gb");
}

fn build_rom() -> Vec<u8> {
    let mut rom = Vec::new();
    
    rom.extend_from_slice(&nintendo_logo());
    rom.extend_from_slice(&[
        b'O', b'N', b'E', b'I', b'T', b'E', b'R', 0, 0, 0, 0, 0, 0, 0, 0, 0,
    ]);
    
    while rom.len() < 0x0147 { rom.push(0); }
    rom.push(0x00);
    while rom.len() < 0x014E { rom.push(0); }
    rom.push(0); rom.push(0);
    
    while rom.len() < 0x0200 { rom.push(0); }
    
    let instructions = game_code();
    for inst in instructions {
        rom.extend_from_slice(&inst.to_bytes());
    }
    
    while rom.len() < 32768 {
        rom.push(0);
    }
    
    rom
}

fn game_code() -> Vec<Instruction> {
    use Instruction::*;
    use U8Register::*;
    use U16Register::*;
    use FlagCondition::*;
    
    vec![
        DI,
        LD_16_IMMEDIATE(SP, 0xFFFE),
        
        // Store 210 at 0xC100-0xC101
        LD_16_IMMEDIATE(HL, 0xC100),
        LD_8_IMMEDIATE(A, 0xD2), // 210 low
        LD_8_INTERNAL(AT_HL, A),
        INC_16(HL),
        LD_8_IMMEDIATE(A, 0x00), // 210 high
        LD_8_INTERNAL(AT_HL, A),
        
        // Load value into DE
        LD_16_IMMEDIATE(HL, 0xC100),
        LD_8_INTERNAL(E, AT_HL),
        INC_16(HL),
        LD_8_INTERNAL(D, AT_HL),
        
        // Output '1'
        LD_8_IMMEDIATE(A, b'1'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        // Divide
    ]
    .into_iter()
    .chain(div_de_by_85_16bit())
    .chain(vec![
        // A now contains remainder (40)
        // BC contains quotient (2)
        
        // Save all results first!
        PUSH_AF,  // Save remainder
        PUSH(BC), // Save quotient
        
        // Output '2' to verify we got here
        LD_8_IMMEDIATE(A, b'2'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        // Restore results
        POP(BC),
        POP_AF,
        
        // Save remainder to 0xC110
        LD_16_IMMEDIATE(HL, 0xC110),
        LD_8_INTERNAL(AT_HL, A),
        
        // Store quotient back to 0xC100-0xC101
        LD_16_IMMEDIATE(HL, 0xC100),
        LD_8_INTERNAL(A, C),
        LD_8_INTERNAL(AT_HL, A),
        INC_16(HL),
        LD_8_INTERNAL(A, B),
        LD_8_INTERNAL(AT_HL, A),
        
        // Output '3'
        LD_8_IMMEDIATE(A, b'3'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        // Newline
        LD_8_IMMEDIATE(A, b'\n'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        HALT,
        JR(-1),
    ])
    .collect()
}

fn div_de_by_85_16bit() -> Vec<Instruction> {
    use Instruction::*;
    use U8Register::*;
    use U16Register::*;
    use FlagCondition::*;
    
    vec![
        LD_16_IMMEDIATE(BC, 0),
        LD_8_INTERNAL(A, D),
        OR(A),
        JR_IF(if_NZ, 5),
        LD_8_INTERNAL(A, E),
        CP_IMMEDIATE(85),
        JR_IF(if_C, 13),
        LD_8_INTERNAL(A, E),
        SUB_IMMEDIATE(85),
        LD_8_INTERNAL(E, A),
        JR_IF(if_NC, 2),
        DEC(D),
        INC_16(BC),
        JR(-19),
        LD_8_INTERNAL(A, E),
    ]
}

fn nintendo_logo() -> [u8; 0x30] {
    [
        0xCE, 0xED, 0x66, 0x66, 0xCC, 0x0D, 0x00, 0x0B,
        0x03, 0x73, 0x00, 0x83, 0x00, 0x0C, 0x00, 0x0D,
        0x00, 0x08, 0x11, 0x1F, 0x88, 0x89, 0x00, 0x0E,
        0xDC, 0xCC, 0x6E, 0xE6, 0xDD, 0xDD, 0xD9, 0x99,
        0xBB, 0xBB, 0x67, 0x63, 0x6E, 0x0E, 0xEC, 0xCC,
        0xDD, 0xDC, 0x99, 0x9F, 0xBB, 0xB9, 0x33, 0x3E,
    ]
}
