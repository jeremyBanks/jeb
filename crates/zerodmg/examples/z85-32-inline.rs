// Inline 32-bit Z85 test - no subroutines, direct division test

use zerodmg_codes::instruction::prelude::*;
use zerodmg_codes::instruction::FlagCondition;

fn main() {
    let rom = build_rom();
    std::fs::write("z85-32-inline.gb", &rom).expect("Failed to write ROM");
    println!("Generated z85-32-inline.gb");
    println!("Test: 210 → '40 2 0 0 0' remainders (displays as ASCII)");
}

fn build_rom() -> Vec<u8> {
    let mut rom = Vec::new();
    
    rom.extend_from_slice(&nintendo_logo());
    rom.extend_from_slice(&[
        b'Z', b'8', b'5', b'-', b'I', b'N', b'L', b'N', 0, 0, 0, 0, 0, 0, 0, 0,
    ]);
    
    while rom.len() < 0x0147 { rom.push(0); }
    rom.push(0x00);
    while rom.len() < 0x014E { rom.push(0); }
    rom.push(0); rom.push(0);
    
    while rom.len() < 0x0200 { rom.push(0); }
    
    let instructions = game_code();
    for inst in instructions {
        rom.extend_from_slice(&inst.to_bytes());
    }
    
    while rom.len() < 32768 {
        rom.push(0);
    }
    
    rom
}

fn game_code() -> Vec<Instruction> {
    use Instruction::*;
    use U8Register::*;
    use U16Register::*;
    use FlagCondition::*;
    
    let mut code = vec![
        DI,
        LD_16_IMMEDIATE(SP, 0xFFFE),
        
        // Store test value: 210 = 0xD2
        LD_16_IMMEDIATE(HL, 0xC100),
        LD_8_IMMEDIATE(A, 0xD2), LD_8_INTERNAL(AT_HL, A), INC_16(HL),
        LD_8_IMMEDIATE(A, 0x00), LD_8_INTERNAL(AT_HL, A), INC_16(HL),
        LD_8_IMMEDIATE(A, 0x00), LD_8_INTERNAL(AT_HL, A), INC_16(HL),
        LD_8_IMMEDIATE(A, 0x00), LD_8_INTERNAL(AT_HL, A),
    ];
    
    // Divide 5 times, output each remainder
    for i in 0..5 {
        // Inline division by 85
        // LOOP_START: Check if value < 85
        code.extend(vec![
            LD_16_IMMEDIATE(HL, 0xC103),
            LD_8_INTERNAL(A, AT_HL), OR(A), JR_IF(if_NZ, 15), // Byte 3 != 0
            
            DEC_16(HL),
            LD_8_INTERNAL(A, AT_HL), OR(A), JR_IF(if_NZ, 10), // Byte 2 != 0
            
            DEC_16(HL),
            LD_8_INTERNAL(A, AT_HL), OR(A), JR_IF(if_NZ, 5), // Byte 1 != 0
            
            DEC_16(HL),
            LD_8_INTERNAL(A, AT_HL),
            CP_IMMEDIATE(85),
            JR_IF(if_C, 26), // < 85, done
            
            // Subtract 85
            LD_16_IMMEDIATE(HL, 0xC100),
            LD_8_INTERNAL(A, AT_HL),
            SUB_IMMEDIATE(85),
            LD_8_INTERNAL(AT_HL, A),
            
            INC_16(HL),
            LD_8_INTERNAL(A, AT_HL),
            SBC_IMMEDIATE(0),
            LD_8_INTERNAL(AT_HL, A),
            
            INC_16(HL),
            LD_8_INTERNAL(A, AT_HL),
            SBC_IMMEDIATE(0),
            LD_8_INTERNAL(AT_HL, A),
            
            INC_16(HL),
            LD_8_INTERNAL(A, AT_HL),
            SBC_IMMEDIATE(0),
            LD_8_INTERNAL(AT_HL, A),
            
            JR(-45), // Loop back
            
            // Done with division, A has remainder
            LD_16_IMMEDIATE(HL, 0xC100),
            LD_8_INTERNAL(A, AT_HL),
        ]);
        
        // Output remainder as ASCII (add '0')
        code.extend(vec![
            ADD_IMMEDIATE(b'0'),
            LD_8_TO_FF_IMMEDIATE(0x01),
            LD_8_IMMEDIATE(A, 0x81),
            LD_8_TO_FF_IMMEDIATE(0x02),
        ]);
        
        // Space between digits
        if i < 4 {
            code.extend(vec![
                LD_8_IMMEDIATE(A, b' '),
                LD_8_TO_FF_IMMEDIATE(0x01),
                LD_8_IMMEDIATE(A, 0x81),
                LD_8_TO_FF_IMMEDIATE(0x02),
            ]);
        }
    }
    
    // Newline
    code.extend(vec![
        LD_8_IMMEDIATE(A, b'\n'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        HALT,
        JR(-1),
    ]);
    
    code
}

fn nintendo_logo() -> [u8; 0x30] {
    [
        0xCE, 0xED, 0x66, 0x66, 0xCC, 0x0D, 0x00, 0x0B,
        0x03, 0x73, 0x00, 0x83, 0x00, 0x0C, 0x00, 0x0D,
        0x00, 0x08, 0x11, 0x1F, 0x88, 0x89, 0x00, 0x0E,
        0xDC, 0xCC, 0x6E, 0xE6, 0xDD, 0xDD, 0xD9, 0x99,
        0xBB, 0xBB, 0x67, 0x63, 0x6E, 0x0E, 0xEC, 0xCC,
        0xDD, 0xDC, 0x99, 0x9F, 0xBB, 0xB9, 0x33, 0x3E,
    ]
}
