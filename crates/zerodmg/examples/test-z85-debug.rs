//! Z85 with debug output after division
...
fn main() {
    let rom = build_rom();
    std::fs::write("test-z85-debug.gb", &rom).expect("Failed to write ROM");
    println!("Generated test-z85-debug.gb");
}
...
fn game_code() -> Vec<Instruction> {
    ...
    .chain(vec![
        // Output BC immediately after division
        PUSH_AF,
        PUSH(BC),
        LD_8_INTERNAL(A, C),
        ADD_IMMEDIATE(48),
