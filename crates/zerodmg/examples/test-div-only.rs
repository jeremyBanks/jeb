//! Test just the division function

use zerodmg_codes::instruction::prelude::*;
use zerodmg_codes::instruction::FlagCondition;

fn main() {
    let rom = build_rom();
    std::fs::write("test-div-only.gb", &rom).expect("Failed to write ROM");
    println!("Generated test-div-only.gb");
}

fn build_rom() -> Vec<u8> {
    let mut rom = Vec::new();
    
    rom.extend_from_slice(&nintendo_logo());
    rom.extend_from_slice(&[
        b'D', b'I', b'V', b'T', b'E', b'S', b'T', 0, 0, 0, 0, 0, 0, 0, 0, 0,
    ]);
    
    while rom.len() < 0x0147 { rom.push(0); }
    rom.push(0x00);
    while rom.len() < 0x014E { rom.push(0); }
    rom.push(0); rom.push(0);
    
    while rom.len() < 0x0200 { rom.push(0); }
    
    let instructions = game_code();
    for inst in instructions {
        rom.extend_from_slice(&inst.to_bytes());
    }
    
    while rom.len() < 32768 {
        rom.push(0);
    }
    
    rom
}

fn game_code() -> Vec<Instruction> {
    use Instruction::*;
    use U8Register::*;
    use U16Register::*;
    
    vec![
        DI,
        LD_16_IMMEDIATE(SP, 0xFFFE),
        
        // Load 210 into DE
        LD_16_IMMEDIATE(DE, 210),
        
        // Output 'B' before division
        LD_8_IMMEDIATE(A, b'B'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        // Call division (210 / 85 should give quotient=2, remainder=40)
    ]
    .into_iter()
    .chain(div_de_by_85_16bit())
    .chain(vec![
        // Save division results FIRST before any output
        PUSH_AF, // Save remainder in A
        PUSH(BC), // Save quotient in BC
        
        // Output 'A' after division
        LD_8_IMMEDIATE(A, b'A'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        // Restore results
        POP(BC),
        POP_AF,
        
        // Store results at 0xC100-0xC102
        LD_16_IMMEDIATE(HL, 0xC100),
        
        // Store remainder (in A) at 0xC100
        LD_8_INTERNAL(AT_HL, A),
        INC_16(HL),
        
        // Store quotient low (C) at 0xC101
        LD_8_INTERNAL(A, C),
        LD_8_INTERNAL(AT_HL, A),
        INC_16(HL),
        
        // Store quotient high (B) at 0xC102
        LD_8_INTERNAL(A, B),
        LD_8_INTERNAL(AT_HL, A),
        
        // Output newline
        LD_8_IMMEDIATE(A, b'\n'),
        LD_8_TO_FF_IMMEDIATE(0x01),
        LD_8_IMMEDIATE(A, 0x81),
        LD_8_TO_FF_IMMEDIATE(0x02),
        
        HALT,
        JR(-1),
    ])
    .collect()
}

fn div_de_by_85_16bit() -> Vec<Instruction> {
    use Instruction::*;
    use U8Register::*;
    use U16Register::*;
    use FlagCondition::*;
    
    vec![
        // Initialize quotient to 0
        LD_16_IMMEDIATE(BC, 0),
        
        // DIV_LOOP: while DE >= 85
        // Check if D > 0 (then definitely >= 85)
        LD_8_INTERNAL(A, D),
        OR(A),
        JR_IF(if_NZ, 5), // If D != 0, skip to subtract
        
        // D == 0, check E >= 85
        LD_8_INTERNAL(A, E),
        CP_IMMEDIATE(85),
        JR_IF(if_C, 13), // If E < 85, exit (jump +13 to LOOP_EXIT)
        
        // DE >= 85, subtract
        LD_8_INTERNAL(A, E),
        SUB_IMMEDIATE(85),
        LD_8_INTERNAL(E, A),
        JR_IF(if_NC, 2), // No borrow
        DEC(D), // Borrow from high byte
        
        // Increment quotient
        INC_16(BC),
        
        // Loop back
        JR(-21), // 19 bytes loop body + 2 for JR
        
        // LOOP_EXIT
        LD_8_INTERNAL(A, E), // Remainder in A
        // BC has quotient
    ]
}

fn nintendo_logo() -> [u8; 0x30] {
    [
        0xCE, 0xED, 0x66, 0x66, 0xCC, 0x0D, 0x00, 0x0B,
        0x03, 0x73, 0x00, 0x83, 0x00, 0x0C, 0x00, 0x0D,
        0x00, 0x08, 0x11, 0x1F, 0x88, 0x89, 0x00, 0x0E,
        0xDC, 0xCC, 0x6E, 0xE6, 0xDD, 0xDD, 0xD9, 0x99,
        0xBB, 0xBB, 0x67, 0x63, 0x6E, 0x0E, 0xEC, 0xCC,
        0xDD, 0xDC, 0x99, 0x9F, 0xBB, 0xB9, 0x33, 0x3E,
    ]
}
