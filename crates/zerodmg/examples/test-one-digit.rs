//! Test one complete digit iteration
...
fn main() {
    let rom = build_rom();
    std::fs::write("test-one-digit.gb", &rom).expect("Failed to write ROM");
    println!("Generated test-one-digit.gb");
}
...
fn build_rom() -> Vec<u8> {
    ...
    rom.extend_from_slice(&[
        b'O', b'N', b'E', b'D', b'I', b'G', b'I', b'T', 0, 0, 0, 0, 0, 0, 0, 0,
    ]);
...
fn game_code() -> Vec<Instruction> {
    ...
    vec![
        ...
        // Set up: value=210 at 0xC100, counter=5 at 0xC105
        ...
        // === ONE DIGIT ITERATION (matching z85-real.rs) ===
    ...
    .chain(vec![
        ...
        // Calculate digit pointer
        LD_16_IMMEDIATE(HL, 0xC10F),
        ADD(L),
        LD_8_INTERNAL(L, A),
        
        // Output what A is now (should be counter+0x0F)
        LD_8_IMMEDIATE(A, b'['),
        ...
        // Store digit
        POP_AF, // Should get remainder=40
        ...
        LD_8_INTERNAL(AT_HL, A), // Store remainder at HL (0xC114)
