//! Check BC before division starts
...
fn main() {
    let rom = build_rom();
    std::fs::write("test-bc-before-div.gb", &rom).expect("Failed to write ROM");
    println!("Generated test-bc-before-div.gb");
}
...
fn build_rom() -> Vec<u8> {
    ...
    rom.extend_from_slice(&[
        b'B', b'C', b'B', b'4', b'D', b'I', b'V', 0, 0, 0, 0, 0, 0, 0, 0, 0,
    ]);
    ...
    let instructions = vec![
        ...
        // Set BC to 999 to see if it gets cleared
        LD_16_IMMEDIATE(BC, 999),
        
        // Output BC before division
        LD_8_IMMEDIATE(A, b'['),
        ...
        // Now call division
        LD_16_IMMEDIATE(BC, 0),  // ← This should reset BC to 0
        LD_8_INTERNAL(A, D),
        OR(A),
        JR_IF(if_NZ, 5),
        LD_8_INTERNAL(A, E),
