...
Figured this out so that I can have an `LD(dest, source)` wrapper function that
works for different registers, for `zerodmg_codes::Instructions`. We probably
won't use it in the `zerodmg_emulator::CPU` at this point, but I'm planning to
use it to generate a demo ROM, so I'd like a nice syntax.

[See this in the Rust Playground](https://play.rust-lang.org/?gist=841464325d2054d1064d2938ffd4e8bd&version=stable&mode=debug&edition=2015)
or below:
