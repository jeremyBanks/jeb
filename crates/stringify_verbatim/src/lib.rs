...
/// Find the base indentation by looking for a group with contaminated outer span
/// but good inner spans.
            ...
            if let Some(first) = inner.first() {
                ...
                if line_diff > 4 {
                    ...
                    if g.delimiter() == proc_macro2::Delimiter::Brace {
                        // For brace groups (function bodies), subtract 4 for the indent level
                        return Some(first_start.column.saturating_sub(4));
                    }
                    // Don't use brackets/parens for base indent - their inner tokens
                    // are mid-line and don't tell us about statement indentation
                }
            }
            // Recurse into groups
