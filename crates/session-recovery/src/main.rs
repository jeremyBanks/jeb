...
fn main() -> Result<()> {
    ...
    for op in &all_ops {
        match &op.kind {
            ...
            OpKind::Edit { old, new } => {
                ...
                let sig = Signature::new(&aname, aemail, &Time::new(op.ts.timestamp(), op.tz))?;
                let t = repo.find_tree(new_tree)?;
                let msg = if ok { format!("edit: {}", ps) } else { format!("⚠️ edit (appended): {}", ps) };
                let pc = repo.find_commit(parent.unwrap())?;
                let oid = repo.commit(None, &sig, &sig, &msg, &t, &[&pc])?;
                parent = Some(oid);
                commits += 1;
    ...
    // Prepare merge with "ours" strategy - keep current tree, just add history
    if let Some(head_id) = orig_head {
        ...
        let slist = if session_ids.len() == 1 { format!("session {}", session_ids[0]) } else { format!("sessions {}", session_ids.join(", ")) };
