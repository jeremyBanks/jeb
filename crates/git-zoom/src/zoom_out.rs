...
fn normalize_path(path: &str) -> String {
    path.split('/').filter(|s| !s.is_empty() && *s != ".").collect::<Vec<_>>().join("/")
}
    ...
    if deny_empty {
        ...
        if entries.is_empty() {
            return Err(git::Error {
                command: "zoom out".to_string(),
                message: "subtree is empty (use without --deny-empty to allow)"
                    .to_string(),
            });
        }
    }
...
mod tests {
    use {
        super::*, crate::zoom_in, std::{fs, process::Command},
        tempfile::TempDir,
    };
    ...
    fn test_parse_target_path() {
        ...
        assert_eq!(
            parse_target_path(Some("abc123")), (Some("abc123".to_string()), None)
        );
        assert_eq!(
            parse_target_path(Some("abc123:src/lib")), (Some("abc123".to_string()),
            Some("src/lib".to_string()))
        );
        assert_eq!(
            parse_target_path(Some(":src/lib")), (None, Some("src/lib".to_string()))
        );
        assert_eq!(
            parse_target_path(Some(":src/lib/")), (None, Some("src/lib".to_string()))
        );
    ...
    fn test_multiple_paths() {
        ...
        Command::new("git")
            ...
            .unwrap();
        zoom_out(None, false).unwrap();
        let lib_content = fs::read_to_string(dir.path().join("src/lib/lib.txt"))
            .unwrap();
        let bin_content = fs::read_to_string(dir.path().join("src/bin/main.txt"))
            .unwrap();
    ...
    fn test_nested_zoom() {
        ...
        assert!(dir.path().join("root.txt").exists());
        let content = fs::read_to_string(dir.path().join("src/lib/core/mod.rs"))
            .unwrap();
        assert_eq!(content, "v2");
    }
    #[test]
    fn test_explicit_target() {
        ...
        let foo_content = fs::read_to_string(dir.path().join("src/lib/foo.txt"))
            .unwrap();
