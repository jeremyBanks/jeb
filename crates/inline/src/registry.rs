...
//! `Box<Mutex<InlineCellInner<T>>>`. The boxes are intentionally leaked to
//! provide true `'static` lifetime. Type safety is ensured by including
//! `TypeId` in the registry key.
//!
//! # Safety
//!
...
///
/// Each entry is a `Box<Mutex<InlineCellInner<T>>>` cast to `usize` for type
...
///
/// **Note:** This is an internal function called by `cell()` and
/// `cell_default()`. Users should use those functions instead.
#[doc(hidden)]
#[track_caller]
pub fn get_or_create<T: Value + 'static>(initial: T) -> &'static Mutex<InlineCellInner<T>> {
