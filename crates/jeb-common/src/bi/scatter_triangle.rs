...
where
    ...
    T::Out: ScatterTriangle<Out = T> + Copy,
{
    let result = value.scatter_triangle_impl();
    #[cfg(fuzzing)]
    {
        let roundtrip = result.scatter_triangle_impl();
        debug_assert_eq!(roundtrip, value, "scatter_triangle roundtrip failed");
    }
    result
}
...
pub trait ScatterTriangle {
    type Out;
    /// Internal implementation - use `scatter_triangle()` function instead for fuzz-checked version
    #[doc(hidden)]
    fn scatter_triangle_impl(self) -> Self::Out;
}
