        ...
        for path in self.entries.keys() {
            ...
            if let Some(slash_pos) = relative.find('/') {
                ...
                if !entries.contains_key(dir_name) {
                    ...
                    entries.insert(dir_name.to_string(), (40000, subtree_hash));
                }
            } else {
                let blob_hash = self
                    .blob_hashes
                    .get(path)
                    .copied()
                    .unwrap_or_else(|| compute_blob_hash(self.entries.get(path).unwrap()));
                entries.insert(relative.to_string(), (100644, blob_hash));
            }
        }
    ...
    match (head_parsed, refs_parsed.is_empty()) {
        ...
        (None, false) => {
            ...
            let head_target = search_order
                ...
                .or_else(|| refs_parsed.keys().min_by_key(|name| name.as_str()).cloned())
                .ok_or(ParseError::MissingField("HEAD"))?;
            Ok((HeadStateOrRef::Symbolic(head_target), refs_parsed))
        }
        (Some(head), true) => match &head {
            HeadStateOrRef::Detached(_) => Ok((head, BTreeMap::new())),
...
fn parse_parents(
    ...
    if let Some(parents_value) = mapping.get(&parents_key) {
        ...
        Ok(parents)
    } else if let Some(prev_ref) = prev_commit_ref {
...
fn serialize_commit(
    ...
    let default_message = match id_style {
        ...
        CommitIdStyle::Hex => format!("commit at {}", commit.committer_date.to_iso8601()),
    };
...
"#;
        ...
        assert_eq!(commit2.tree.get("c.txt"), Some("new"));
    }
    #[test]
...
"#;
        ...
        let commit2 = repo.commits().find(|c| c.message == "commit 2").unwrap();
        assert_eq!(commit2.tree.get("a.txt"), Some("a"));
        assert_eq!(commit2.tree.get("b.txt"), None);
    }
    #[test]
