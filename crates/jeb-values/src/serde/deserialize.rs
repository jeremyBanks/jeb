use {
    crate::{serde::{Error, error::Unexpected}, Bytes, Text, Value},
    indexmap::IndexMap,
...
impl<'de> de::Deserializer<'de> for Value {
        ...
        V: Visitor<'de>,
    {
        match self {
            ...
            _ => Err(Error::invalid_type(self.unexpected(), &"an unsigned integer")),
        }
    }
...
impl<'de> de::VariantAccess<'de> for VariantDeserializer {
    ...
    {
        match self.value {
            Some(Value::TextMap(m)) => {
                visitor.visit_map(TextMapDeserializer::new(m))
            }
            Some(Value::BytesMap(m)) => {
                visitor.visit_map(BytesMapDeserializer::new(m))
            }
            Some(Value::Array(arr)) => {
                visitor.visit_map(PairsDeserializer::new(arr))
            }
