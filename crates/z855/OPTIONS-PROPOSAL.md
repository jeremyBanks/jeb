(An AI formatted this up like a concrete proposal but the details are not actually final or authoritative,
just take them as an example illustrating the high-level concept being proposed, but we are free to make
other choices.)
...
- Only standard 4-byte-aligned Z85 encoding used (unless concatenatable mode is enabled)
...
4. This uses Z85 alphabet characters but is **not compatible with pure Z85** encoding
...
- Decoder change required regardless of encoder option (must support concatenatable output)
