...
let q=X>0?G(X):"";o+=q+g+"|"+".".repeat(X);for(let j=0;j<w;j++)o+=C(b[i+j]);let p=d-q[L]-X
...
let h=H(v,p+1),j=_e(k);o+=h+j;for(let l=0;l<k;l++)o+=C(b[s+l]);u+=p+k+2;i+=p+k;T=1;break}if(T)break}}if(T)continue}
...
for(let j=0;j<e;j++)z[P](b[w+j]);let A=$(z,0),g=H(v,p);o+=g+",";for(let j of y)o+=C(j)
o+=W(A,5-p);u+=10;i+=8;V=1;break}if(V)continue}if(r>=4){let v=$(b,i);o+=b85(v,5);u+=5;i+=4}else{let m=r,c=m+1,v=m==1?b[i]:m==2?(b[i]<<8)|b[i+1]:(b[i]<<16)|(b[i+1]<<8)|b[i+2];o+=b85(v,c);u+=c;i+=m}}return o},decode=(s)=>{let S=s[L];if(!S)return new U(0);let q=j=>K(s[j]),W=(g,e)=>{let v=0,m=1,p=e,c=0;while(p>0){p--;c++;let d=g[p]
