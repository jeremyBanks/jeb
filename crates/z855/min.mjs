...
TypeError('invalid input')},U=Uint8Array,D=[],K=s=>s.charCodeAt(),H=new Set([...Z+",;|~_"].map(c=>K(c))),$=(a,i)=>((a[i]<<24)|(a[i+1]<<16)|(a[i+2]<<8)|a[i+3])>>>0
for(let j=E;j--;)D[K(Z[j])]=j
// CLI: node/deno/bun min.mjs <encode|decode> <stdin >stdout
if(import.meta.main||import.meta.url===`file://${process?.argv?.[1]}`){(async()=>{let I=typeof Deno!=='undefined',a=I?Deno.args:process?.argv||Bun?.argv||[],m=I?a[0]:a[a[0]?.includes?.('node')||a[0]?.includes?.('bun')?2:1];if(!m||!['encode','decode'].includes(m)){console.error('usage: min.mjs <encode|decode> <stdin >stdout');(process||Deno||Bun).exit(2)}let d='',e=new TextEncoder(),r=new TextDecoder();if(I){Deno.stdin.setRaw?.(false);for await(let c of Deno.stdin.readable){d+=r.decode(c,{stream:true})}}else{process.stdin.setEncoding('utf8');for await(let c of process.stdin)d+=c}d=d.trimEnd();let o=m==='encode'?z855(e.encode(d)):r.decode(decode(d));I?Deno.stdout.writeSync(typeof o==='string'?e.encode(o):o):(process?.stdout||Bun?.stdout)?.write?.(o)})()}
