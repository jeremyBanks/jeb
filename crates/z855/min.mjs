...
o+=H(v,p+1)+(k<6?";":k<7?"_":"~");for(let l=0;l<k;l++)o+=C(b[s+l]);u+=p+k+2;i+=p+k;T=1;break}if(T)break}}if(T)continue}
...
for(let j=0;j<4-p;j++)z[P](b[i+4+p+j]);o+=H(v,p)+",";for(let j of y)o+=C(j)
...
if(import.meta.main||import.meta.url===`file://${process?.argv?.[1]}`){(async()=>{let D=typeof Deno!=='undefined',a=D?Deno.args:process?.argv||Bun?.argv||[],m=D?a[0]:a[a[0]?.includes?.('node')||a[0]?.includes?.('bun')?2:1];if(!m||!['encode','decode'].includes(m))((process||Deno||Bun)?.exit?.(1));let d='',e=new TextEncoder(),r=new TextDecoder();if(D){Deno.stdin.setRaw?.(false);for await(let c of Deno.stdin.readable){d+=r.decode(c,{stream:true})}}else{process.stdin.setEncoding('utf8');for await(let c of process.stdin)d+=c}d=d.trimEnd();let o=m==='encode'?z855(e.encode(d)):r.decode(decode(d));D?Deno.stdout.writeSync(typeof o==='string'?e.encode(o):o):(process?.stdout||Bun?.stdout)?.write?.(o)})()}
