...
if(import.meta.main||import.meta.url===`file://${process?.argv?.[1]}`){(async()=>{let I=typeof Deno!=='undefined',a=I?Deno.args:process?.argv||Bun?.argv||[],m=I?a[0]:a[a[0]?.includes?.('node')||a[0]?.includes?.('bun')?2:1];if(!m||!['encode','decode'].includes(m))((process||Deno||Bun)?.exit?.(1));let d='',e=new TextEncoder(),r=new TextDecoder();if(I){Deno.stdin.setRaw?.(false);for await(let c of Deno.stdin.readable){d+=r.decode(c,{stream:true})}}else{process.stdin.setEncoding('utf8');for await(let c of process.stdin)d+=c}d=d.trimEnd();let o=m==='encode'?z855(e.encode(d)):r.decode(decode(d));I?Deno.stdout.writeSync(typeof o==='string'?e.encode(o):o):(process?.stdout||Bun?.stdout)?.write?.(o)})()}
