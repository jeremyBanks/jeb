...
mod tests {
    ...
    async fn test_lines_switches_between_text_and_bytes() {
        ...
        let mixed_source = stream! {
            yield Ok(Item::Text("hello\n".into())); yield Ok(Item::Bytes(b"world\n"
            .to_vec().into())); yield Ok(Item::Text("foo\n".into()));
        };
    ...
    async fn test_chunks_switches_between_text_and_bytes() {
        use async_stream::stream;
        let mixed_source = stream! {
            yield Ok(Item::Text("12345".into())); yield Ok(Item::Bytes(b"67890".to_vec()
            .into()));
        };
    ...
    async fn test_lines_propagates_errors() {
        ...
        let error_source = stream! {
            yield Ok(Item::Text("hello\n".into())); yield Err("test error"); yield
            Ok(Item::Text("world\n".into()));
        };
    ...
    async fn test_chunks_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Text("12345".into())); yield Err("test error"); yield
            Ok(Item::Text("67890".into()));
        };
    ...
    async fn test_parse_hex_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Text("aa".into())); yield Err("test error"); yield
            Ok(Item::Text("bb".into()));
        };
    ...
    async fn test_split_whitespace_mixed_types() {
        use async_stream::stream;
        let mixed_source = stream! {
            yield Ok(Item::Text("hello world".into())); yield Ok(Item::Bytes(b"foo bar"
            .to_vec().into()));
        };
    ...
    async fn test_split_whitespace_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Text("hello".into())); yield Err("test error"); yield
            Ok(Item::Text("world".into()));
        };
    ...
    async fn test_collapse_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Text("hello  world".into())); yield Err("test error"); yield
            Ok(Item::Text("foo   bar".into()));
        };
    ...
    async fn test_filter_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Text("hello".into())); yield Ok(Item::Text("".into())); yield
            Err("test error"); yield Ok(Item::Text("world".into()));
        };
    ...
    async fn test_to_binary_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Bytes(vec![0xFF] .into())); yield Err("test error");
        };
    ...
    async fn test_parse_binary_invalid_bit_count() {
        let source = text_source(vec!["1111111".to_string()]);
    ...
    async fn test_parse_binary_propagates_errors() {
        use async_stream::stream;
        let error_source = stream! {
            yield Ok(Item::Text("11111111".into())); yield Err("test error"); yield
            Ok(Item::Text("00000000".into()));
        };
