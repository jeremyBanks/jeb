...
pub fn stdin() -> impl Stream<Item = Result<Item, &'static str>> + Send + Unpin {
    Box::pin(stream! {
        let mut reader = tokio::io::stdin(); let mut buffer = [0u8; 65_536]; loop {
        match reader.read(& mut buffer). await { Ok(0) => break, Ok(n) => yield
        Ok(Item::Bytes(buffer[..n].to_vec().into())), Err(_) => { yield
        Err("failed to read stdin"); break; } } }
    })
}
...
{
    Box::pin(stream! {
        match tokio::fs::File::open(path). await { Ok(mut reader) => { let mut buffer
        = [0u8; 65_536]; loop { match reader.read(& mut buffer). await { Ok(0) =>
        break, Ok(n) => yield Ok(Item::Bytes(buffer[..n].to_vec().into())), Err(_) =>
        { yield Err("failed to read"); break; } } } } Err(_) => { yield
...
{
    stream! {
        for text_item in text { yield Ok(Item::Text(text_item.into())); }
    }
}
...
{
    stream! {
        for bytes_item in bytes { yield Ok(Item::Bytes(bytes_item.into())); }
    }
}
