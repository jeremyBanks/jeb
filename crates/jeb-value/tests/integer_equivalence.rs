...
fn test_mixed_integer_ordering() {
    let vals = vec![
        Value::Unsigned(10), Value::Signed(- 5), Value::Signed(10), Value::Unsigned(0),
        Value::Signed(0), Value::Unsigned(100),
    ];
...
fn test_integer_float_mixed_ordering() {
    let mut vals = vec![
        Value::Float(5.0.try_into().unwrap()), Value::Unsigned(5), Value::Signed(5),
        Value::Float(4.0.try_into().unwrap()), Value::Unsigned(6),
    ];
    vals.sort();
    match &vals[0] {
...
fn test_edge_case_stable_sort() {
    let mut vals = vec![
        Value::Float(Float::new(0.0).unwrap()), Value::Signed(- 1), Value::Unsigned(0),
        Value::Float(Float::new(- 0.0).unwrap()), Value::Signed(0),
...
fn test_negative_zero_float_ordering() {
    ...
    assert!(neg_zero < pos_zero, "-0.0 should be less than 0.0 in total ordering");
}
#[test]
fn test_stable_sort_order_preservation() {
    let mut vals_a = vec![
        Value::Signed(- 1), Value::Unsigned(0), Value::Signed(0),
        Value::Float(Float::new(5.0).unwrap()),
    ];
    let mut vals_b = vec![
        Value::Signed(- 1), Value::Signed(0), Value::Unsigned(0),
        Value::Float(Float::new(5.0).unwrap()),
    ];
    vals_a.sort();
    vals_b.sort();
    ...
    assert!(
        ! matches!((& vals_a[1], & vals_b[1]), (Value::Unsigned(_), Value::Unsigned(_)) |
        (Value::Signed(_), Value::Signed(_))),
...
fn test_stable_sort_with_floats() {
    let mut vals_a = vec![
        Value::Float(Float::new(0.0).unwrap()), Value::Float(Float::new(- 0.0).unwrap()),
    ];
    let mut vals_b = vec![
        Value::Float(Float::new(- 0.0).unwrap()), Value::Float(Float::new(0.0).unwrap()),
    ];
