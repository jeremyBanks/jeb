...
impl<'de> Visitor<'de> for ValueVisitor {
    ...
    where
        E: de::Error,
    {
        Ok(Value::Boolean(Boolean::from(value)))
    }
    ...
    where
        E: de::Error,
    {
        // Check if exactly representable in f64
        let as_f64 = value as f64;
        if as_f64.is_finite() && (as_f64 as i64) == value {
            Ok(Value::Number(Number(as_f64)))
        } else {
        ...
        E: de::Error,
    {
        // Check if exactly representable in f64
        let as_f64 = value as f64;
        if as_f64.is_finite() && (as_f64 as u64) == value {
            Ok(Value::Number(Number(as_f64)))
        } else {
            Ok(Value::Bytes(Bytes::from(value.to_be_bytes().to_vec())))
        }
        ...
        E: de::Error,
    {
        if value.is_finite() {
            ...
            // Non-finite floats fall back to bytes representation
            Ok(Value::Bytes(Bytes::from(value.to_be_bytes().to_vec())))
        }
    }
    ...
    where
        E: de::Error,
    {
        Ok(Value::String(String::from(value.to_string())))
    }
                ...
                match key {
                    ...
                    _ => {
                        return Err(de::Error::custom(
                            "inconsistent map key types: expected all String keys",
                        ));
                    }
                }
            }
