...
log.debug("Decrypted local Chrome encryption key.");
...
for (const profile of chromeProfiles) {
  ...
  if (!profile.email.match(/nobody|stadia/)) {
    continue;
  }

  const cookieCreds = Object.fromEntries(
    (await db(
      SQL
...
for (
  const [email, googleSession] of Object.entries(
    credentials,
  ) as any
) {
  ...
  console.log(email, userInfo[5][0].join("#"));
}
